...
@section('page-title', 'Kelola User – Tambah Pengelola')
...
<div class="pt-4">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <button form="form-admin" type="submit"
        ...
        <div class="px-6 py-5">
            <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-4">TAMBAH PENGELOLA</h3>

            <form id="form-admin" method="POST" action="{{ route('admin.user.store-admin') }}" class="space-y-4">
                @csrf

                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">NAMA LENGKAP</label>
                    <input type="text" name="FullName" value="{{ old('FullName') }}" placeholder="Nama lengkap"
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">NUPTK</label>
                    <input type="number" name="NISNUPTK" value="{{ old('NISNUPTK') }}" placeholder="Nomor NUPTK"
                        class="w-full border border-gray-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none
                               focus:ring-2 focus:ring-[#1a2e5a]/30 focus:border-[#1a2e5a]
